...
/*
	Define a parser to parse the assembly code.
	Use "recursive descent parser" algorithm to parse the assembly code.
...
func New(input string, symbolTable *symboltable.SymbolTable) *Parser {
	parser := &Parser{input: input, commandStrList: strings.Split(input, token.NEW_LINE), currentCommandIdx: 0, readPosition: 0, SymbolTable: symbolTable}
	return parser
}
...
func (p *Parser) parseACommand() (*ast.ACommand, error) {
	...
	return &ast.ACommand{StringValue: valueStr, Value: value}, nil
}
...
func (p *Parser) Symbol() (string, error) {
	...
	case ast.A_COMMAND:
		...
		p.resetReadPosition()
		return aCommand.StringValue, nil
	case ast.L_COMMAND:
...
func (p *Parser) removeWhiteSpace() {
	p.commandStrList[p.currentCommandIdx] = strings.Replace(p.commandStrList[p.currentCommandIdx], string(token.SPACE), "", -1)
	p.commandStrList[p.currentCommandIdx] = strings.Replace(p.commandStrList[p.currentCommandIdx], string(token.TAB), "", -1)
}
